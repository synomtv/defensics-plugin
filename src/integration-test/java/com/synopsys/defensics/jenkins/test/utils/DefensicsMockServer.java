...
public class DefensicsMockServer {
   ...
   * somewhere  else as well but currently there isn't suitable util method so let's have this here
   * for now.
   *
  ...
  public static void stopMockServer(ClientAndServer mockServer) {
    mockServer.stop();
    // Windows CI builds fails sometimes due to "Unable to connect to socket" so wait a bit
    // that previous test server has stopped and released its socket.
